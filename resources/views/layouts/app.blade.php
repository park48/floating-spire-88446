...
<!-- <div class="container">
  <header class="header">
    <a href="{{ url('/')}}" class="header-title">team3.com</a>
    <a href="{{ url('/users')}}" class="header-menu">ユーザー一覧</a>
    <a href="{{ url('/login')}}" class="header-menu">ログイン</a>
    <a class="header-menu" href="{{ route('logout') }}" onclick="event.preventDefault();document.getElementById('logout-form').submit();">
      ログアウト
    </a>
    <form id="logout-form" action="{{ route('logout') }}" method="POST" style="display: none;">
      {{ csrf_field() }}
    </form>
    <a href="{{ url('/register')}}" class="header-menu">新規登録</a>
  </header>
</div> -->
  ...
  <nav class="nav">
    <ul class="header_menu">
      ...
      <li class="header_menu_title">
          <a class="nav-link listNew" href="/">team3.com</a>
      </li>
      <li>
        <ul class="header_menu_inner">
          <li>
              <a class="nav-link listNew" href="{{ route('login') }}">リストを作成</a>　　
          </li>
          <li>
